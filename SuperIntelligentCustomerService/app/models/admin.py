...
class AuditLog(BaseModel, TimestampMixin):
    ...
    status = fields.IntField(default=0, description="状态码", index=True)
...
class ChatSession(BaseModel, TimestampMixin):
    """聊天会话模型"""
    session_title = fields.CharField(max_length=200, description="会话标题", index=True)
    session_content = fields.TextField(null=True, description="会话内容")
    user_id = fields.IntField(description="用户ID", index=True)
...
class ChatMessage(BaseModel, TimestampMixin):
    ...
    model_name = fields.CharField(max_length=100, null=True, description="模型名称", index=True)
    total_tokens = fields.IntField(default=0, description="累计Tokens")
    deduct_cost = fields.DecimalField(max_digits=10, decimal_places=4, default=0, description="扣除金额")
    remark = fields.CharField(max_length=500, null=True, description="备注")
