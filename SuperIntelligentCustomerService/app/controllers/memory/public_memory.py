...
class PublicMemoryService(BaseMemoryService):
    """
    公共知识库服务
    基于ChromaDB向量数据库和重排模型实现，继承BaseMemoryService抽象基类
    """
    ...
    def __init__(self):
        ...
        """
        # 调用父类初始化
        service_id = "public_memory_global"
        super().__init__(service_id, MemoryType.PUBLIC)
    ...
    async def _add_memory_vector(self, memory_id: str, content: str, metadata: Dict[str, Any]) -> str:
        ...
        try:
            ...
            doc_metadata = {
                ...
                "priority": str(metadata.get("priority", 1)),
                "is_active": str(metadata.get("is_active", True)),
                "created_at": datetime.now().isoformat(),
                "content_type": "knowledge_base"
            }
    ...
    async def retrieve_memories(self, query: str, limit: int = 5, use_reranker: bool = True) -> List[MemoryItem]:
        """检索相关公共知识，使用向量检索+重排模型"""
        # 确保知识库已初始化
        await self._ensure_knowledge_initialized()
        return await self._retrieve_memories_vector(query, limit, use_reranker)

    async def _retrieve_memories_vector(self, query: str, limit: int = 5, use_reranker: bool = True) -> List[MemoryItem]:
        """使用向量数据库和重排模型检索记忆"""
        try:
            # 生成查询嵌入
    ...
    async def query(self, query: str, limit: int = 5, **kwargs) -> QueryResult:
        ...
        try:
            ...
            memories = await self.retrieve_memories(query, limit, use_reranker)
            ...
            return QueryResult(
                ...
                metadata={
                    "query_type": "public_knowledge_search",
        ...
        try:
            ...
            if category:
                ...
                if results["ids"]:
                    ...
                    logger.info(f"已清空公共知识库分类: {category}")
            else:
                # 清空所有数据
                ...
                try:
                    ...
                    if result['ids']:
                        ...
                        logger.info(f"已清空所有公共知识库数据，共 {len(result['ids'])} 条记录")
                    else:
                        logger.info("公共知识库已经为空")
                except Exception as delete_error:
                    # 如果获取ID失败，尝试删除整个集合并重新创建
                    logger.warning(f"无法获取文档ID，尝试重新创建集合: {delete_error}")
                    self.chroma_client.delete_collection(self.collection_name)
                    self.collection = self.chroma_client.create_collection(name=self.collection_name)
                    logger.info("已重新创建公共知识库集合")
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            logger.info(f"公共记忆服务 {self.service_id} 已关闭")
        except Exception as e:
            logger.error(f"关闭公共记忆服务时发生错误: {e}")
