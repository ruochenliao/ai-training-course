#!/usr/bin/env node
/**
 * 企业级Agent+RAG知识库系统 - 统一前端应用启动脚本
 * 基于 Next.js 框架的现代化企业级前端应用
 */
...
class UnifiedAppServer {
    ...
    async checkDependencies() {
        ...
        // 检查关键依赖
        const requiredDeps = ['next', 'react', 'react-dom', 'antd'];
...
`;
    ...
    async startProductionServer() {
        ...
        this.log(`生产服务地址: http://${host}:${port}`);
    ...
    async runTypeCheck() {
        ...
        return new Promise((resolve) => {
            ...
            typeCheckProcess.on('error', (err) => {
                this.log(`类型检查出错: ${err.message}`, 'error');
                resolve(false);
            });
        });
    }
    ...
    async runTests() {
        ...
        return new Promise((resolve) => {
            ...
            testProcess.on('error', (err) => {
                this.log(`测试出错: ${err.message}`, 'error');
                resolve(false);
            });
        });
    }
     ...
     */
    async run(mode = 'dev') {
        console.log(chalk.magenta('🎯 企业级Agent+RAG知识库系统 - 统一前端应用'));
...
function main() {
    ...
    if (!['dev', 'build', 'start', 'type-check', 'test'].includes(mode)) {
        console.log(chalk.red('❌ 无效的模式'));
        console.log(chalk.yellow('💡 支持的模式: dev, build, start, type-check, test'));
        process.exit(1);
    }
