...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    def _init_default_validation_rules(self):
        ...
        self.validation_rules = [
            ...
            # 命令注入检测 - 优化规则，避免误判User-Agent
            ValidationRule(
                name="command_injection",
                pattern=r"(?i)(?<!user-agent:\s)(?<!mozilla/)(?<!chrome/)(?<!safari/)(?<!edge/)(;|\||&|`|\$\(|wget\s|curl\s|nc\s|netcat\s|bash\s|sh\s|cmd\s|powershell\s)",
                message="检测到潜在的命令注入攻击",
                security_level=SecurityLevel.CRITICAL
            ),

            # LDAP注入检测 - 优化后的规则，避免误判正常路径
            ValidationRule(
                name="ldap_injection",
                pattern=r"(?i)(\*\)|&\(|\|\(|!\(|=\*\)|>=\*|<=\*|~=\*|\(\*|\)\*|null\)|nil\)|objectclass=\*)",
        ...
        # User-Agent白名单模式 - 常见浏览器和工具的User-Agent模式
        self.user_agent_whitelist_patterns = [
            r"Mozilla/.*",  # 所有Mozilla系列浏览器
            r"Chrome/.*",   # Chrome浏览器
            r"Safari/.*",   # Safari浏览器
            r"Edge/.*",     # Edge浏览器
            r"Firefox/.*",  # Firefox浏览器
            r"Opera/.*",    # Opera浏览器
            r"curl/.*",     # curl工具
            r"wget/.*",     # wget工具
    ...
    async def _validate_input_security(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查是否为系统路径白名单，如果是则跳过安全验证
            if self._is_system_path(path):
                return {"valid": True, "reason": "系统路径白名单，跳过安全验证"}
            ...
            for header_name, header_value in request.headers.items():
                if header_name.lower() in ["user-agent", "referer", "x-forwarded-for"]:
                    # 特殊处理User-Agent：检查是否在白名单中
                    if header_name.lower() == "user-agent":
                        if self._is_user_agent_whitelisted(header_value):
                            continue  # 跳过白名单中的User-Agent
    ...
    def _is_system_path(self, path: str) -> bool:
        """检查是否为系统路径"""
        # 精确匹配系统路径
        if path in self.system_paths_whitelist:
            return True

        # 检查是否以系统路径开头（支持带参数的路径）
        for system_path in self.system_paths_whitelist:
            if path.startswith(system_path):
                # 确保是完整路径匹配，避免误匹配
                remaining = path[len(system_path):]
    ...
    def _is_user_agent_whitelisted(self, user_agent: str) -> bool:
        """检查User-Agent是否在白名单中"""
        if not user_agent:
            return False

        for pattern in self.user_agent_whitelist_patterns:
