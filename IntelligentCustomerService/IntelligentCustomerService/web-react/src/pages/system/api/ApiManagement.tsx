      ...
      render: (method: string) => {
        ...
        return (
          <Tag 
            ...
            className="rounded-md py-1 px-2 uppercase font-medium"
          >
            {method}
          </Tag>
        );
    ...
    },
    {
      ...
      render: (_: any, record: ApiItem) => (
        <Space>
          ...
          <Popconfirm
            title="确定删除此API吗？"
            description={<>删除后不可恢复，确认删除<b>{record.path}</b>吗？</>}
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.code === 200) {
        ...
        // 使用类型断言处理错误信息
        const errorMsg = (response as any).msg || (response as any).message || 
          (currentApi ? '更新API失败' : '添加API失败');
        message.error(errorMsg);
      }
    } catch (error) {
