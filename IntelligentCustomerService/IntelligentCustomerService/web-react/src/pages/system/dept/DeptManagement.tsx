...
import { DeleteOutlined, EditOutlined, FolderOutlined, PlusOutlined, ReloadOutlined, SearchOutlined } from '@ant-design/icons'
...
const DeptManagement: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'remark',
      ellipsis: true,
      render: (remark: string) => <div className='text-gray-600'>{remark || '-'}</div>,
    },
    {
      title: '操作',
      key: 'actions',
      ...
      render: (_: any, record: Dept) => (
        <Space>
          <Button type='link' size='small' icon={<PlusOutlined />} onClick={() => handleAddSubDept(record)}>
            添加下级
          </Button>
          ...
          <Popconfirm
            ...
            description={
              <>
                删除后不可恢复，确认删除<b>{record.name}</b>吗？
              </>
            }
  ...
  return (
    <div className='dept-management h-full'>
      <Card className={cn('h-full rounded-lg overflow-hidden', isDark ? 'bg-gray-800 border-gray-700' : 'bg-white')} bordered={false}>
        {/* 搜索表单 */}
        <div className='mb-4'>
          <Form form={searchForm} layout='inline' onFinish={handleSearch} className='gap-4 flex-wrap' style={{ rowGap: '12px' }}>
            <Form.Item name='name' label='部门名称'>
              <Input placeholder='请输入部门名称' allowClear />
            </Form.Item>
            <Form.Item>
              <Space>
                <Button type='primary' htmlType='submit' icon={<SearchOutlined />} style={{ backgroundColor: primaryColor }}>
                  搜索
                </Button>
        ...
        <div className='mb-4 flex justify-between'>
          <div className={cn('text-lg font-medium', isDark ? 'text-white' : 'text-gray-800')}>部门列表</div>
          <Space>
            <Button type='primary' icon={<PlusOutlined />} onClick={handleAdd} style={{ backgroundColor: primaryColor }}>
              新增部门
            </Button>
            <Button icon={<ReloadOutlined />} onClick={() => fetchDeptList()}>
              刷新
            </Button>
          </Space>
        </div>
      ...
      >
        <Form form={form} layout='vertical' requiredMark={false}>
          <Form.Item name='parent_id' label='上级部门' rules={[{ required: true, message: '请选择上级部门' }]}>
            <TreeSelect
              treeData={[{ title: '顶级部门', value: 0 }, ...convertToTreeData(deptList)]}
              placeholder='请选择上级部门'
              style={{ width: '100%' }}
