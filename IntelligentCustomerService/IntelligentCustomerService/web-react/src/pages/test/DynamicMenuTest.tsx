...
import { Card, Descriptions, Tag, Tree, Spin, Alert } from 'antd';
    ...
    return rawData.map(item => ({
      title: (
        <span>
          ...
          {item.is_hidden && <Tag color="red" style={{ marginLeft: 4 }}>隐藏</Tag>}
        </span>
      ),
      key: item.id,
    ...
    <div style={{ padding: '24px' }}>
      <Card title="🎯 动态菜单测试页面 - 参考vue-fastapi-admin设计" style={{ marginBottom: '24px' }}>
        ...
        <Descriptions title="菜单统计信息" bordered column={2}>
          <Descriptions.Item label="菜单总数">
            {menus.length}
          </Descriptions.Item>
          <Descriptions.Item label="原始菜单数据">
            {rawMenus.length}
          </Descriptions.Item>
          <Descriptions.Item label="用户权限数量">
            {permissions.length}
          </Descriptions.Item>
          <Descriptions.Item label="加载状态">
            <Tag color={menuLoading ? 'processing' : 'success'}>
              {menuLoading ? '加载中' : '已加载'}
            </Tag>
          </Descriptions.Item>
        </Descriptions>
      ...
      <Card title="📋 转换后的菜单数据 (Ant Design Menu格式)" style={{ marginBottom: '24px' }}>
        {menus.length > 0 ? (
          <>
            <Alert
              message="路由测试"
              description="点击下方菜单项可以测试路由跳转是否正确"
              type="info"
      ...
      <Card title="🔧 原始菜单数据 (API返回格式)" style={{ marginBottom: '24px' }}>
        {rawMenus.length > 0 ? (
          <Tree
            treeData={convertRawToTreeData(rawMenus)}
            defaultExpandAll
            showLine
            showIcon={false}
          />
