          ...
          {import.meta.env.DEV && this.state.errorInfo && (
            <div className='mt-4 p-4 bg-gray-100 rounded overflow-auto max-h-96'>
              <details>
                ...
                <pre className='whitespace-pre-wrap text-sm text-red-600'>{this.state.error && this.state.error.toString()}</pre>
                <pre className='whitespace-pre-wrap text-xs text-gray-600 mt-2'>{this.state.errorInfo.componentStack}</pre>
              </details>
            </div>
          )}
        </Result>
