...
import DynamicForm, {DynamicFormRef, FormItemConfig} from './DynamicForm.tsx'
...
const QueryBar = React.forwardRef<QueryBarRef, QueryBarProps>(
  (
    { items, onSearch, onReset, loading = false, defaultCollapsed = false, showCollapse = true, columns = 3, searchText, resetText, initialValues },
    ref,
  ) => {
    ...
    // 根据折叠状态决定显示的表单项
    const visibleItems = showCollapse && collapsed ? items.slice(0, columns) : items
