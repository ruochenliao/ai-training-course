...
import {type Role, roleApi, type RoleCreateData, type RoleUpdateData} from '@/api/role.ts'
import {type Menu, menuApi} from '@/api/menu.ts'
import {apiApi, type ApiItem} from '@/api/api.ts'
...
const RoleManagement: React.FC = () => {
  ...
  const filterTreeData = (data: MenuTreeNode[], searchText: string): MenuTreeNode[] => {
    ...
    const filterNode = (node: MenuTreeNode): MenuTreeNode | null => {
      ...
      const filteredChildren = node.children
        ? (node.children.map((child) => filterNode(child as MenuTreeNode)).filter(Boolean) as MenuTreeNode[])
        : []
  ...
  const filterApiTreeData = (data: ApiTreeNode[], searchText: string): ApiTreeNode[] => {
    ...
    const filterApiNode = (node: ApiTreeNode): ApiTreeNode | null => {
      ...
      const pathMatch = node.path && node.path.toLowerCase().includes(searchLower)
      const filteredChildren = node.children
        ? (node.children.map((child) => filterApiNode(child as ApiTreeNode)).filter(Boolean) as ApiTreeNode[])
        : []
  ...
  const handleConfigPermission = async (record: Role) => {
    ...
    try {
      ...
      // 获取所有菜单和API
      const [menuResponse, apiResponse] = await Promise.all([menuApi.getMenuTree(), apiApi.getApis()])
    ...
    },
    {
      ...
      render: (_: any, record: Role) => (
        <Space>
          ...
          <Button type='link' size='small' icon={<EditOutlined />} onClick={() => handleEditRole(record)} className='action-button'>
            编辑
          </Button>
          {record.name !== 'admin' && (
            <Button
  ...
  const roleFormContent = (
    <Form form={form} layout='vertical' requiredMark={false} className='system-form'>
      ...
      <Form.Item name='desc' label='角色描述'>
        <Input.TextArea placeholder='请输入角色描述' rows={4} maxLength={200} showCount />
      </Form.Item>
    </Form>
  )
    ...
    <div className='role-management' style={{ padding: '24px' }}>
      ...
      >
        ...
        <div className='mb-4 flex justify-between'>
          <div className={cn(isDark ? 'text-white' : 'text-gray-800')}>角色列表</div>
          <Space>
            <Button type='primary' icon={<PlusOutlined />} onClick={handleAddRole} className='add-button' style={{ backgroundColor: primaryColor }}>
              新增角色
            </Button>
            <Button icon={<ReloadOutlined />} onClick={() => fetchRoleData()} className='reset-button'>
              刷新
            </Button>
          </Space>
        </div>
      ...
      {/* 角色表单弹窗 */}
      <Modal title={modalTitle} open={modalVisible} onOk={handleModalOk} onCancel={() => setModalVisible(false)} width={550} className='system-modal'>
        {roleFormContent}
      </Modal>
      ...
      <Drawer
        ...
        footer={
          <div style={{ textAlign: 'right' }}>
            ...
            <Button type='primary' onClick={handlePermissionOk} style={{ backgroundColor: primaryColor }}>
              保存
            </Button>
          </div>
        }
      ...
      >
        <Input.Search placeholder='搜索权限' allowClear onChange={(e) => setSearchPattern(e.target.value)} className='mb-4' />
          ...
          items={[
            {
              ...
              label: '菜单权限',
              children: (
                <div className={cn('border rounded-lg p-4', isDark ? 'border-gray-700' : 'border-gray-200')}>
                  <Tree
                    checkable
                    defaultExpandAll
