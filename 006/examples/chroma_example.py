...
# collection.add(
#     documents=docs,
#     metadatas=[{"catalog": "账户与安全"}, {"catalog": "商品与浏览"}, {"catalog": "下单与支付"}, {"catalog": "订单状态与物流"}, {"catalog": "退换货与售后"}, {"catalog": "其它问题"}],
#     ids=["id1", "id2", "id3", "id4", "id5", "id6"],
# )

# 基于大模型对用户问题进行处理：1、识别用户意图，准确标记元数据的过滤规则，2、识别用户问题，抽取部分关键字，做关键字查询，3、改写用户问题
# 微调嵌入模型--成本低很多
results = collection.query(
    query_texts=["支付方式有哪些"],
    n_results=2,    # 建议给到最大（召回的数据不要超过大模型的上下文尺寸上限）
    # where={"catalog": "退换货与售后"},
    # where_document={"$or":[{"$contains": "退货"}, {"$contains": "申请"}]} # 全文检索
)
print(results)
