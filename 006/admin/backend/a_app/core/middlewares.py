...
from a_app.models.admin import AuditLog, User
...
class HttpAuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_args(self, request: Request) -> dict:
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            ...
            # For multipart/form-data (file uploads), skip JSON parsing
            if content_type.startswith("multipart/form-data"):
                try:
                    # Don't try to update args with form data for file uploads
                    # as it will be handled by the endpoint
                    pass
                except Exception as e:
                    print(f"Error processing multipart form: {e}")
            else:
                # For other content types, try JSON first then form
                try:
